...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="aida0/guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'main', url:'https://github.com/ghk01214/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
        ...
        stage('Staging Deploy') {
            ...
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@172.17.0.2 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@172.17.0.2 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=172.17.0.2 \
                                        -e MYSQL_PORT=3306 \
                                        -e MYSQL_DATABASE=guestbook \
                                        -e MYSQL_USER=root \
                                        -e MYSQL_PASSWORD=4869 \
                                        ${strDockerImage} "
                }
            }
        }
